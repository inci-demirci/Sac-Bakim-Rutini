...
import com.example.sabakmrutinim.databinding.FragmentCtsBinding
import kotlin.getValue

class CtsFragment : Fragment() {
    private lateinit var binding: FragmentCtsBinding
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentCtsBinding.inflate(inflater, container, false)
        binding.recyclerViewCumartesi.layoutManager = LinearLayoutManager(requireContext())

        val urunListesi=viewModel.getGunlukUrunler(sacTipi!!,"cumartesi")
        adapter = UrunAdapter(
        ...
        binding.recyclerViewCumartesi.adapter=adapter
        binding.textViewBaslik.text=when(sacTipi){
            "kuru"->"Isısız Şekillendirme"
            "normal"->"Isısız Şekillendirme"
            "yağlı"->"Isısız Şekillendirme+Sprey"
            else -> ""
        }
        return binding.root
    }
