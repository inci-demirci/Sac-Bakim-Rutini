...
import com.example.sabakmrutinim.databinding.FragmentCumBinding
import kotlin.getValue

class CumFragment : Fragment() {
    private lateinit var binding: FragmentCumBinding
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentCumBinding.inflate(inflater, container, false)
        binding.recyclerViewCuma.layoutManager = LinearLayoutManager(requireContext())

        val urunListesi=viewModel.getGunlukUrunler(sacTipi!!,"cuma")
        adapter = UrunAdapter(
        ...
        binding.recyclerViewCuma.adapter=adapter
        binding.textViewBaslik.text=when(sacTipi){
            "kuru"->"Yoğun Nem Maskesi"
            "normal"->"Yoğun Nemlendirme"
            "yağlı"->"Maske Günü"
            else -> ""
        }
        return binding.root
